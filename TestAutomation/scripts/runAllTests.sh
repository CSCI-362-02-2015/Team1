...
echo '<link rel="stylesheet" type="text/css" href="report.css">' >> testReport.html
echo "<h1>Test Results</h1>" >> testReport.html
echo "<table style='border: 1px solid black; border-collapse: collapse'><tr><th>TestID</th><th>Requirement</th><th>Inputs</th><th>Expected Output</th><th>Test Result</th></tr>" >> testReport.html
...
while [ $num -le $numTest ]; do
	...
	while read p; do
	    if [[ ${p:0:1} == D* ]];
			...
			if [[ $result == Passed ]];
				then echo '<th><font color="green"> '$result' </font></th>' >> ../reports/testReport.html;
				echo -e Test $num ${GREEN}$result${NC}
			fi 
			
			if [[ $result == Failed ]];
				then echo '<th><font color="red"> '$result' </font></th>' >> ../reports/testReport.html;
		...
		if [[ ${p:0:1} == T* ]];
			then echo '<th>'${p:8}'</th>' >> ../reports/testReport.html;		#put the various test case information into the report
		fi

		if [[ ${p:0:1} == R* ]];
			then echo '<th>'${p:12}'</th>' >> ../reports/testReport.html;
		fi

		if [[ ${p:0:1} == I* ]];
			then echo '<th>'${p:8}'</th>' >> ../reports/testReport.html;
			input=$p
		fi

		if [[ ${p:0:1} == E* ]];
			then echo '<th>'${p:10}'</th>' >> ../reports/testReport.html;
			output=$p
		fi
	done <$var
	num=$((num+1))
